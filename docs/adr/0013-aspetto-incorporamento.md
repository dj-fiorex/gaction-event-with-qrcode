...
- I token si scrivono su `:root`, non sul contenitore del form. `body` porta `bg-background text-foreground`, quindi risolve `var(--background)` e `var(--foreground)` **su di sé**, che sta sopra il contenitore: definendo le variabili più in basso, body resta col tema predefinito e ogni discendente privo di una classe di colore propria — i titoli delle sezioni del form — eredita quel nero **già calcolato**. Le variabili si ereditano; il valore che un `var()` ha già prodotto no. Sbagliarlo non rompe tutto in modo evidente: ciò che ha una classe di colore (`text-muted-foreground`, `bg-primary`) si tematizza lo stesso, e resta nero solo ciò che eredita.
- Il carattere fa eccezione e si applica come `font-family` inline sul contenitore: in Tailwind v4 il blocco `@theme inline` incorpora il valore dentro l'utility `font-sans`, che quindi non rilegge `--font-sans`, e una regola su `body` perderebbe comunque contro la specificità di una classe.
